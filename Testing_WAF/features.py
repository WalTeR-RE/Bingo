...
def compute_stats(s):
    s = str(s)
    length = len(s)
    spec = sum(1 for c in s if c in SPECIAL)
    ...
    sql_kw = len(re.findall(r"(?i)(union|select|insert|drop|or|and|exec|sleep)", s))
    base = [
...
def build_matrix(texts, tfidf):
    X_tfidf = tfidf.transform(texts)
    X_stat = sparse.csr_matrix(stats_frame(texts))
